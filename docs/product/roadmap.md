...
截至 2026-09-24，P0-2 基线为 `96ef1d5`，阶段 1.1、1.2、1.3 已在其上完成，开发分支为 `dev`。
...
| 阶段 1.1 / P0-3A 基础贡献审核 | 已完成 | 新资源/基础纠错提议、本人历史与撤回、审核修订采纳、基础额度、原子落库 |
| 阶段 1.2 / P0-3B 完整贡献类型 | 已完成 | 来源新增/失效移除、标签、双端关系与翻译提议，统一审核、历史及原子采纳 |
| 阶段 1.3 / P0-6 举报与治理 | 已完成 | 私密举报与处理、范围限制、固定信任额度、原子审计、人工来源核查及推荐资格 |
| 阶段 2.1～2.2 / P0-4～P0-5 | 待完成 | 搜索发现、收藏整理与完整产品界面 |
| P0-7 整体收尾 | 待完成 | 完整产品联调、生产准备、恢复演练与上线验收 |

现有能力已打通「七种贡献 → 修订采纳 → 发布 → 浏览／访问来源 → 举报 → 治理 → 作者查看安全结果」。
阶段 1.3 的权限／并发、浏览器闭环、迁移及共享 dev smoke 见[验收记录](../implementation/p0-6-verification.md)。
阶段 1.1 的实际验证见[验收记录](../implementation/p0-3a-verification.md)。
阶段 1.2 的浏览器、并发/隐私、迁移兼容与真实开发 smoke 见[验收记录](../implementation/p0-3b-verification.md)。
...
```text
当前：P0-2、阶段一（1.1～1.3）已完成；下一步 2.1 搜索与发现
    ↓
...
**状态：1.1、1.2、1.3 已完成。目标：用户能贡献，运营能审核、追踪和处理问题。**

| 子阶段 | 集中交付范围 | 可独立验收的结果 |
| --- | --- | --- |
| 1.1 基础贡献审核（已完成） | 新资源与基础字段修改提议、本人贡献列表/详情、差异查看、后台队列、接受/拒绝/撤回、审核历史和基础额度 | 普通用户提交，Editor/Admin 审核；接受后原子落库，拒绝或撤回不改变资源，重复处理和旧版本冲突不会覆盖数据 |
| 1.2 完整贡献类型（已完成） | 已有资源的来源新增/失效移除、添加标签、添加关系与资源翻译提议；复用 1.1 的审核和历史 | 所有 P0 贡献类型都能从页面提交、审核并正确影响资源；来源保留、关系双端版本/环检查、语言回退和隐私均保持正确 |
| 1.3 举报与治理（已完成） | 举报处理、范围限制、基础信任/额度、内容展示资格、治理审计、来源健康记录及后台运营入口 | 举报有处理结果，限制即时生效并保留举报/账号安全渠道，治理操作可追踪，来源问题可处理；为后续搜索/推荐提供统一资格规则 |

阶段一的产品规则、复用边界和重点验收见[阶段一设计](stage-1-contribution-governance.md)。
该文档覆盖三个子阶段；1.1 已按[基础贡献审核规格](../implementation/p0-3a-basic-contribution-review.md)完成前后端与验收。
1.2 已按[完整贡献类型规格](../implementation/p0-3b-complete-contribution-types.md)集中补齐五种新提议、
双端关系版本、翻译编辑与迁移兼容，并完成前后端及验收。
1.3 的[设计前审计](../implementation/p0-6-baseline-audit.md)与[举报治理规格](../implementation/p0-6-reports-governance.md)
已实施并验收：资料写入事务内授权、举报处理与原子治理、限制／信任额度及运营页面均已接通。
业务限制保留举报、登录、账号安全和本人记录；来源核查采用人工记录，不引入自动巡检。
推荐资格已实现为可复用的纯策略与 SQL 判断；本阶段未做搜索/排名页面。共享 dev 已向上迁移到 9，旧迁移与既有权限保持。
阶段一正式收口，接下来进入 2.1 搜索与发现；仍按完整 P0 完成后才正式上线。
...
**状态：阶段一已完成，下一步设计／实施 2.1。目标：用户能找到、理解、整理资源，并进入贡献流程。**
