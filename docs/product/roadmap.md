...
截至 2026-09-23，P0-2 基线为 `96ef1d5`，阶段 1.1 已在其上完成，开发分支为 `dev`。
...
| 阶段 1.2 / P0-3B 完整贡献类型 | 设计中，未实施 | 已审计 1.1 基线，细化来源、标签、关系和翻译提议 |
| 阶段 1.3～2.2 / P0-4～P0-6 其余能力 | 待完成 | 举报治理、搜索发现、收藏整理 |
| P0-7 整体收尾 | 待完成 | 完整产品联调、生产准备、恢复演练与上线验收 |

现有能力已打通「用户提出资源/纠错 → 后台修订采纳 → 发布 → 用户浏览 → 访问来源」。
...
**状态：1.1 已完成，1.2 设计中。目标：用户能贡献，运营能审核、追踪和处理问题。**
...
| 1.2 完整贡献类型（设计中） | 已有资源的来源新增/失效移除、添加标签、添加关系与资源翻译提议；复用 1.1 的审核和历史 | 所有 P0 贡献类型都能从页面提交、审核并正确影响资源；来源保留、关系双端版本/环检查、语言回退和隐私均保持正确 |
...
1.2 的[完整贡献类型设计稿](../implementation/p0-3b-complete-contribution-types.md)细化五种新提议、双端关系版本、翻译回退与迁移兼容；
集中补齐其余贡献类型，不为每种提议另开一个项目。设计完成不代表功能已经交付。
