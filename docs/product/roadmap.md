...
截至 2026-09-23，P0-2 基线为 `96ef1d5`，阶段 1.1、1.2 已在其上完成，开发分支为 `dev`。
...
| 阶段 1.3 / P0-6 举报与治理 | 设计完成，未实施 | 已审计 1.2 基线并确定举报、范围限制、信任额度、审计和人工来源核查边界 |
| 阶段 2.1～2.2 / P0-4～P0-5 | 待完成 | 搜索发现、收藏整理与完整产品界面 |
| P0-7 整体收尾 | 待完成 | 完整产品联调、生产准备、恢复演练与上线验收 |

现有能力已打通「用户提交七种贡献 → 后台修订采纳 → 发布 → 用户浏览 → 访问来源」。
...
**状态：1.1、1.2 已完成，1.3 设计完成待实施。目标：用户能贡献，运营能审核、追踪和处理问题。**
...
| 1.3 举报与治理（设计完成） | 举报处理、范围限制、基础信任/额度、内容展示资格、治理审计、来源健康记录及后台运营入口 | 举报有处理结果，限制即时生效并保留举报/账号安全渠道，治理操作可追踪，来源问题可处理；为后续搜索/推荐提供统一资格规则 |
...
已落地：先补资料写入的事务内授权/限制接入，再集中实现举报到治理结果的闭环。
业务限制保留举报、登录、账号安全和本人记录；来源核查采用人工记录，不引入自动巡检。
本阶段只设计推荐资格，不提前做搜索/排名页面；预计 migration 9，当前代码/数据库仍为 8。
设计完成不计作治理功能交付；1.3 实施验收后再进入 2.1 搜索与发现。
