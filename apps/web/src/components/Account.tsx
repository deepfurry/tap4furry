...
import { BusinessStatus } from './reports/Reports';
...
export default function Account() {
  ...
  useEffect(() => {
    ...
    void getMe({ signal: controller.signal, credentials: 'same-origin', cache: 'no-store' })
      .then((response) => {
      ...
      .catch(() => {
        if (!controller.signal.aborted)
          setMessage('Unable to load your account. Please reload to try again.');
      })
      .finally(() => {
  ...
  async function save(event: SubmitEvent<HTMLFormElement>) {
    ...
    if ((displayName && [...displayName].length > 80) || (bio && [...bio].length > 500)) {
      setMessage('Use at most 80 characters for your display name and 500 for your bio.');
      return;
    }
    setBusy(true);
    setMessage('');
      ...
      const response = await updateProfile(
        {
          ...
          bio,
          search_engine_indexing: values.has('search_engine_indexing'),
        },
  ...
  if (unauthenticated)
    return (
      <p>
        <a href="/login">Log in</a> or <a href="/register">create an account</a> to manage your
        profile.
      </p>
    );
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-2">
        ...
        <p className={styles.hint}>
          {me.email_verified ? 'Email verified' : 'Email not yet verified'}. Your email stays
          private.
        </p>
      </div>
      <BusinessStatus
        onRestricted={setProfileRestricted}
        onMe={(value) => {
          setMe(value);
          setRevision((v) => v + 1);
        }}
      ...
      <form key={revision} onSubmit={save} className="flex flex-col gap-6">
        <fieldset disabled={profileRestricted || busy} className="grid gap-6">
          <label className="flex flex-col gap-2">
            ...
            <input
              ...
              spellCheck={false}
            />
            <span id="handle-help" className={styles.hint}>
              Optional. 3–32 lowercase letters, numbers, underscores or hyphens; start with a letter
              or number.
            </span>
          </label>
          <label className="flex flex-col gap-2">
            ...
            />
            <span id="name-help" className={styles.hint}>
              Up to 80 characters.
            </span>
          </label>
          <label className="flex flex-col gap-2">
            ...
            />
            <span id="bio-help" className={styles.hint}>
              Up to 500 characters. Your handle, display name and bio are public.
            </span>
          </label>
          <label className="flex items-start gap-3">
            <input
              ...
              type="checkbox"
              defaultChecked={me.profile.search_engine_indexing}
            />
