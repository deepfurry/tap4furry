...
import { AdminRequestError, errorMessage, requireAdmin, writeOptions } from '../lib/admin-api';
...
export function AccountPage() {
  ...
  const mutation = useMutation({
    mutationFn: async (action: {
      kind: 'logout' | 'reauth' | 'others' | 'revoke';
      id?: string;
      current?: boolean;
    }) => {
      ...
      const response =
        action.kind === 'logout'
          ...
          : action.kind === 'reauth'
            ? await reauthenticate({ password: suppliedPassword }, options)
            : action.kind === 'others'
              ? await revokeOtherSessions(options)
              : await revokeSession(action.id!, options);
      if (response.status !== 204)
        throw new AdminRequestError(response.status, response.data.message, response.data.code);
      if (action.kind === 'logout' || action.current) {
        client.clear();
    ...
    onError: async (error) => {
      if (
        ...
        (error.code === 'ADMIN_UNAUTHENTICATED' || error.code === 'ADMIN_FORBIDDEN')
      ) {
        client.clear();
        await navigate({ to: '/login' });
      }
    ...
    <main className="mx-auto flex min-h-screen max-w-3xl flex-col gap-6 p-6 sm:p-10">
      <header className="flex flex-wrap items-center justify-between gap-4">
        ...
        <button
          ...
          onClick={() => mutation.mutate({ kind: 'logout' })}
        >
          Sign out
        </button>
      </header>
      ...
      <section
        className={`${styles.panel} flex flex-col gap-3 p-6`}
        ...
        {me.isPending ? (
          ...
          me.data && (
            <>
              ...
              <p>Roles: {me.data.roles.join(', ')}</p>
              <p>Authenticated: {new Date(me.data.authenticated_at).toLocaleString()}</p>
            </>
          )
        )}
      </section>
      <section
        className={`${styles.panel} flex flex-col gap-5 p-6`}
        aria-labelledby="sessions-heading"
      >
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h2 id="sessions-heading">Admin sessions</h2>
          <button
            className={`${styles.button} px-4 py-2`}
            disabled={mutation.isPending || !sessions.data}
            onClick={() => mutation.mutate({ kind: 'others' })}
          >
        ...
        {sessions.isPending ? (
          ...
          <ul className="flex flex-col gap-4">
            {sessions.data?.map((session) => (
              ...
              >
                <div>
                  <p>{session.current ? 'This session' : 'Another session'}</p>
                  <p>Last active: {new Date(session.last_seen_at).toLocaleString()}</p>
                  <p>Expires: {new Date(session.absolute_expires_at).toLocaleString()}</p>
                </div>
                <button
                  className={`${styles.button} px-3 py-2`}
                  disabled={mutation.isPending}
                  onClick={() =>
                    mutation.mutate({ kind: 'revoke', id: session.id, current: session.current })
                  }
                >
                  Revoke{session.current ? ' current' : ''}
                </button>
              </li>
            ))}
          </ul>
      ...
      >
        <h2 id="reauth-heading">Verify your identity again</h2>
        <p>Confirm your password to refresh this session.</p>
        <form
          className="flex flex-col gap-4"
          onSubmit={(event) => {
            event.preventDefault();
            mutation.mutate({ kind: 'reauth' });
          }}
        >
